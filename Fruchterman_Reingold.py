...
def run():
    ...
    area = 800*500
    k = math.sqrt(area/len(g.nodes.keys())) 
    ...
    for n in g.nodes.keys():
        if g.getNode(n).coordinate[0] == 0 and  g.getNode(n).coordinate[1] == 0:
            ...
            g.getNode(n).coordinate[0] = float(g.getNode(n).coordinate[0])*20 + 200 # x factor
            g.getNode(n).coordinate[1] = float(g.getNode(n).coordinate[1])*20 + 200 # y factor (Erase the 200 for Grid Graphs)
    ...
    def draw():
        ...
        for nodeName in nodes: 
            ...
            nx = A[0]
            ny = A[1]
            # Max displacement limit
            nx = min(800,max(0,nx)) - 800/2
            ny = min(500,max(0,ny)) - 500/2
            node.gamex = min(math.sqrt(800*800/4-ny*ny),max(-math.sqrt(800*800/4-ny*ny),nx)) + 800/2  
            node.gamey = min(math.sqrt(500*500/4-nx*nx),max(-math.sqrt(500*500/4-nx*nx),ny)) + 500/2
